...
import { createMemorialAsAdminAction } from "@/app/admin/create-memorial-admin-action";
import { MEMORIAL_STYLE_OPTIONS } from "@/lib/memorial-styles";
import { HOSTING_PLANS, PLAN_LABELS } from "@/lib/plans";

export function AdminMemorialForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await createMemorialAsAdminAction(fd);
      if ("error" in result) {
    ...
    >
      ...
      <div className="rounded-2xl border border-foreground/10 bg-background/80 px-6 py-5 shadow-sm">
        ...
        <p className="mt-2 text-foreground">
          Create a memorial without Stripe checkout. You will be added as the
          memorial owner so you can manage it from the family hub if needed.
        </p>
      </div>
      ...
      <div className="space-y-6">
          ...
          >
            {HOSTING_PLANS.map((plan) => (
              <option key={plan} value={plan}>
                {PLAN_LABELS[plan].title} — {PLAN_LABELS[plan].price}{" "}
                {PLAN_LABELS[plan].cadence}
              </option>
            ))}
          </select>
        </label>
        ...
        <label className="block">
          ...
          >
            <option value="published">Published (live on /remember/…)</option>
